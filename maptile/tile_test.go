...
func TestAt(t *testing.T) {
	tile := At(geo.Point{0, 0}, 28)
	if b := tile.Bound(); b.Top() != 0 || b.Left() != 0 {
		t.Errorf("incorrect tile bound: %v", b)
	}

	// specific case
	if tile := At(geo.Point{-87.65005229999997, 41.850033}, 20); tile.X != 268988 || tile.Y != 389836 {
		t.Errorf("projection incorrect: %v", tile)
	}

	if tile := At(geo.Point{-87.65005229999997, 41.850033}, 28); tile.X != 68861112 || tile.Y != 99798110 {
		t.Errorf("projection incorrect: %v", tile)
	}
	...
	// test polar regions
	if tile := At(geo.Point{0, 89.9}, 30); tile.Y != 0 {
		t.Errorf("top of the world error: %d != %d", tile.Y, 0)
	}

	if tile := At(geo.Point{0, -89.9}, 30); tile.Y != (1<<30)-1 {
...
func TestTileBound(t *testing.T) {
	...
	expected := geo.Bound{Min: geo.Point{-180, -85.05112877980659}, Max: geo.Point{180, 85.05112877980659}}
